...
static int gold_value(int lev, bool explode);
...
struct object_kind *select_poss_kind(struct poss_item *poss, int level, int tval)
{
	...
	for (; poss; poss = poss->next) {
		if ((tval == 0) || (tval == k_info[poss->kidx].tval)) {
			 ...
			 * The probability in the usual table is zero, because it doesn't get allocated
			 * normally. Fill in something appropriate.
			 */
			if (k_info[poss->kidx].tval == TV_GOLD) {
				if (k_info[poss->kidx].sval == money_kind(NULL, gold_value(level, true))->sval)
					newprob = 1.0;
			}
			newprob *= poss->scale;
						   ...
						   bool extra_roll, s32b *value, int tval, const char *name)
{
	...
	/* Cash is a special case (it can be generated by chests) */
	if (new_obj->tval == TV_GOLD) {
		do {
			new_obj->pval = gold_value(lev, true);
		} while (new_obj->sval != money_kind(NULL, new_obj->pval)->sval);
	}
...
struct object *make_gold(int lev, const char *coin_type)
{
	...
	int value = gold_value(lev, false);
