...
static void project_feature_handler_HALLU(project_feature_handler_context_t *context)
{
	/* Grid is in line of sight and player is not blind */
	if (square_isview(cave, context->grid) && !player->timed[TMD_BLIND]) {
		/* Observe */
...
static void project_feature_handler_MON_PAINT(project_feature_handler_context_t *context)
{
}
